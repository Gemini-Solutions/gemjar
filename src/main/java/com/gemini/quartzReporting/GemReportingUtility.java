		...
		try {
			...
			if (reportLoc == null) {
				...
				FileUtils.writeStringToFile(
						new File(
								"Report/" + dtf.format(now) + "/" + "GemEcoTestReport_" + dmyhms.format(now) + ".html"),
						htmlTemplate, Charset.defaultCharset());
			} else {
				FileUtils.writeStringToFile(new File(reportLoc + "/GemEcoTestReport.html"), htmlTemplate,
						Charset.defaultCharset());
			}
		} catch (IOException e) {
